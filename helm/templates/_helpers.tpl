...
*/}}
{{- define "chart.s3RegistrySecretName" -}}
{{- .Values.s3Registry.secretName | default (printf "%s-s3-registry" .Release.Name) -}}
{{- end -}}

{{/*
  Resolve the EPP tokenizer (vllm-render) container image.

  The tokenizer sidecar only tokenizes prompts and runs on the (CPU-only) EPP
  pod, so it must use a CPU vLLM build — NOT the GPU model-server image, which
  fails with "Failed to infer device type" on a node without a GPU. Uses
  router.tokenizer.image (registry/repository/tag) directly.
*/}}
{{- define "llm-d-router.tokenizerImage" -}}
{{- $img := (.Values.router.tokenizer | default dict).image | default dict -}}
{{- printf "%s/%s:%s" $img.registry (required "router.tokenizer.image.repository is required" $img.repository) (required "router.tokenizer.image.tag is required" $img.tag) -}}
{{- end -}}
...
{{/* ===================================================================
   llm-d-router (EPP) helpers — vendored from upstream
   github.com/llm-d/llm-d-router (routerlib + standalone + gateway charts)
   The standalone-chart-name guard (hasPrefix "llm-d-router-standalone"
   .Chart.Name) is replaced with `true` so proxy presets resolve under
   this chart's name (vllm-stack).
   =================================================================== */}}
...
{{/*
Resolve the EPP model-server selector labels (matchLabels).

If router.modelServers.matchLabels is set explicitly, use it as-is.
Otherwise auto-derive it from the serving engine: the vLLM pods are labeled
`model: <modelSpec.name>` (see deployment-vllm-multi.yaml), so select the first
enabled modelSpec by that label. Returns a YAML map (use with fromYaml/nindent).
*/}}
{{- define "llm-d-router.resolvedMatchLabels" -}}
{{- $ms := .Values.router.modelServers | default dict -}}
{{- if $ms.matchLabels -}}
{{- toYaml $ms.matchLabels -}}
{{- else -}}
{{- $model := .Values.servingEngineSpec.modelSpec | default dict -}}
{{- if and $model.name (ne $model.enabled false) -}}
model: {{ $model.name | quote }}
{{- end -}}
{{- end -}}
{{- end -}}

{{/*
Resolve the EPP precise-prefix-cache token-processor blockSize.

Single source of truth is the engine's vLLM `--block-size` arg (so the EPP
KV-block index lines up with vLLM's KV cache). Scan the model's
vllmConfig.extraArgs for `--block-size <n>`; fall back to
router.precisePrefixCache.blockSize (default 64) when not found.
*/}}
{{- define "llm-d-router.resolvedBlockSize" -}}
{{- $blockSize := "" -}}
{{- $model := .Values.servingEngineSpec.modelSpec | default dict -}}
{{- if ne $model.enabled false -}}
  {{- $args := (($model.vllmConfig | default dict).extraArgs | default list) -}}
  {{- range $i, $a := $args -}}
    {{- if and (eq (toString $a) "--block-size") (lt (add1 $i) (len $args)) -}}
      {{- $blockSize = toString (index $args (add1 $i)) -}}
    {{- end -}}
  {{- end -}}
{{- end -}}
{{- if eq $blockSize "" -}}
{{- $blockSize = toString (.Values.router.precisePrefixCache.blockSize | default 64) -}}
{{- end -}}
{{- $blockSize -}}
{{- end -}}
...
{{- if not (include "llm-d-router.resolvedMatchLabels" . | trim) }}
{{- fail ".Values.router.modelServers.matchLabels is required (and could not be auto-derived: no enabled servingEngineSpec.modelSpec found)" }}
{{- end }}
{{- end }}
{{- end -}}
...
{{/* ===== Latency predictor helpers (moved from _epp.tpl) ===== */}}
